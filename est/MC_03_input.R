...
# Vehicles
veh <- readRDS('veh/MC_G_250_750.rds') 
# Profiles
# Mileage, Check name of categories with names(fkm)
data(fkm)
lkm <- units::set_units(fkm$KM_MOTO_E25(1:ncol(veh)), km)
# Euro
euro <- c(rep("III", length(2017:2007)),
...
directory <- "MC_03"
vfuel <- 'G' 
vsize <- '50' # It can be small/big/<=1400, one word
vname <-  "MC_250_750" 
type_emi <- "Hot"
